...
public enum OnboardingRouter {
    ...
    public static func nextStep(after current: OnboardingStep, answers: OnboardingAnswers) -> OnboardingStep? {
        ...
        case .cardioActivityPicker, .cardioSession, .cardioHistory,
            ...
            // (plan 04.1-13), and `.completionLogging` (plan 04.1-14) are entered by explicit user
            // choice from the hub or `SettingsView`, never by this router advancing into them --
            // terminal exactly like `.home`, with no branching on answers, age, or any other
            // state. See this file's header comment for the full record.
            return nil
        }
    }
