...
| Physical-device and AX3/AX5 accessibility verification (the checkpoints execute-phase's automated environment structurally cannot perform — real GPS/motion sensors, on-device round-trips, visual/legibility judgment) is batched to run once, after every phase's code is built, rather than gating each phase's close-out individually | Direct product decision (2026-09-06): the user wants to keep building across phases without a manual-testing interruption at every phase boundary, then do one dedicated testing pass at the end. Phase 2's plan 02-16 checkpoint (10 physical-device steps + 7 AX3/AX5 steps) is the first deferred instance — see its SUMMARY.md. Each subsequent phase's own human-verify checkpoints should be deferred the same way unless the user says otherwise; phases stay in `blocked`/pending-verification status in STATE.md until the batched pass happens, but planning/execution of later phases proceeds regardless | Decided |
