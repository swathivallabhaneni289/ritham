...
Fixed as part of Phase 3's code-review pass (`03-REVIEW.md` CR-01), which independently
re-discovered this exact defect with a concrete failure trace and found it was worse than
originally scoped here: not limited to a rare multi-week catch-up, but a **permanent, every-call**
streak-zeroing once any window ever expires unclaimed. Added `ComebackWindow.resolvedAt` (gating
the expiry transition to fire exactly once) plus a related fix to the claim branch, which had its
own unconditional-overwrite clobber bug. See `03-REVIEW-FIX.md` CR-01 for the full fix, formula
derivation, and regression tests (`MomentumReconciliationTests.swift`,
`expiredUnclaimedWindowDoesNotRepeatedlyResetStreakOnLaterMetWeeks`,
`claimingWindowInSameCallAsIndependentlyMetWeekDoesNotClobberHigherStreak`). Commits: `9c7e7ae`,
`3fe745e`.
