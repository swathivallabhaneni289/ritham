...
func TestHandleWorkoutPlan_ValidRequestReturns200WithPlanKey(t *testing.T) {
	...
	NewMux(nil, nil, nil, nil, nil, nil, nil).ServeHTTP(rec, req)
...
func TestNewMux_IdentityRoutesRegisteredOnlyWhenServiceProvided(t *testing.T) {
	...
	t.Run("real service registers identity routes behind RequireSession", func(t *testing.T) {
		...
		NewMux(idsvc, nil, nil, nil, nil, nil, nil).ServeHTTP(rec, req)
...
func TestNewMux_FriendsRoutesRegisteredOnlyWhenServiceProvided(t *testing.T) {
	idsvc := identity.New(nil, noopAppleKeySource{}, "com.ritham.app", time.Now)

	t.Run("nil friends service leaves friends routes unregistered", func(t *testing.T) {
	...
	t.Run("real service registers friends routes behind RequireSession", func(t *testing.T) {
		friendssvc := friends.New(nil, time.Now, []byte("test-salt"))
		req := httptest.NewRequest(http.MethodGet, "/v1/friends", nil)
		rec := httptest.NewRecorder()

		NewMux(idsvc, nil, nil, friendssvc, nil, nil, nil).ServeHTTP(rec, req)
