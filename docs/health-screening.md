...
- **13+ age floor (updated 2026-08-23 — see also REQUIREMENTS.md MINOR-01 and `.planning/phases/01-onboarding-safety-intake/01-CONTEXT.md` D-14/D-15):** Ritham requires users to be 13 or older, permanently. Age is self-attested with no verification beyond entry. Under 13 → a plain blocking message ("Ritham is for ages 13+") and the user goes back to re-enter a different age; nothing is saved for the rejected attempt. 13 or older → onboarding continues with full, identical access from that moment on — calibration, tracking, Momentum, and the complete rest of this questionnaire (§1.2 onward, including the SCOFF screen), with zero parental involvement at any age or any point in the flow. This replaces an earlier tiered parental-consent design (under-13 locked pending verifiable consent; 13–17 gated on the sensitive screening behind parent approval) that this document previously described — dropped after tracing its cost (a dedicated backend service, COPPA "email plus" compliance, a legal review) against the problem it solved, and after confirming no competitor fitness app (Strava, Nike, Peloton, MyFitnessPal) builds real under-13 support either; they all use a hard age floor instead. This is a routing outcome, not a UX fork — it never produces a distinct "kid mode" screen, per the no-age-gated-fork principle in §CROSSGEN-05.
