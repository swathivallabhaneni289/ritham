...
1. **Comparison stress.** Strava's group challenges still surface a "Top Contributors" leaderboard underneath the collective goal; Peloton's per-class leaderboard drew documented demotivation and distrust. The implementations that avoided this (Apple Fitness's capped 1:1 with individualized scoring, Racery's pooled team avatar, Strava's frequency-over-speed Local Legends) all share one move: decouple "did I personally succeed" from "how do I compare to someone else." Ritham goes further than any of them — see §2.
...
That's it. The target lives on the **event**, never as a per-person measured output. This is the single sentence that carries the whole philosophy: *the completion card says "completed the Saturday 5K Walk," never "5.4 km in 58:12."* Ritham doesn't capture pace or elapsed time for group display at all — not withheld, not hidden behind a toggle, simply never surfaced to any shared view.
...
**Why Ritham goes flatter than every implementation in the research:** Strava's group challenges still show a "Top Contributors" leaderboard beneath the collective total; Racery's Uniteam mode pools effort into one shared team avatar but still exposes contribution level to teammates; Apple Fitness scores a percentage-of-personal-ring, which is individualized but still a score with a winner. Ritham has none of these — no pooled total, no contribution ranking, no score, no winner. A single shared event where everyone's own 5K counts equally doesn't need a shared number to feel collective; the group's already-agreed target *is* the shared thing, and the group feed of celebrations *is* the shared experience.

**No "first to complete."** Completions render in the feed as they arrive (a normal chronological social feed, not a race clock) — and this is safe specifically *because* there's no starting gun. Reasoning worth keeping explicit in-product: early-vs-late only means "who finished faster" when everyone started at the same instant, which never happens here. But the *card itself* never carries an ordinal position, badge, or "1st" marker regardless — the reasoning above justifies chronological posting order, not chronological *ranking*.
...
| Completion confirmation (to the user) | "You did it! 🎉 Logged for the Saturday 5K Walk." |
| Group feed card | "Priya completed the Saturday 5K Walk." *(no time, no distance, no rank)* |
...
**Explicitly not on it:** finish time, pace, distance-as-measured, place/rank within the group, exact GPS coordinates or address, and — critically for the export case below — anyone else's name, photo, or participation status, and no "X of Y completed" figure.
